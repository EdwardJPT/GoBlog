...
func NewWritingsHandler(db *sql.DB) *WritingsHandler {
	...
	tmpl = template.Must(tmpl.ParseGlob("web/templates/blog/*.html"))
	...
	if len(blogData) > 0 {
		...
		if hasPrev {
			prevCursorBase64 = base64.URLEncoding.EncodeToString(
				[]byte(firstTimestamp.UTC().Format(helpers.SqliteTimeLayout)),
			)
		}
		// Logic for Next Cursor
		if hasNext {
			nextCursorBase64 = base64.URLEncoding.EncodeToString(
				[]byte(lastTimestamp.UTC().Format(helpers.SqliteTimeLayout)),
			)
		}
	}
...
func (h *WritingsHandler) Post(w http.ResponseWriter, r *http.Request) {
	...
	postContent := PostContent{
		Title:       post.Title,
		Content:     helpers.RenderMarkdown(post.Content),
		Tags:        post.Tags,
...
func (h *WritingsHandler) Note(w http.ResponseWriter, r *http.Request) {
	...
	noteContent := NoteContent{
		Title:       note.Title,
		Content:     helpers.RenderMarkdown(note.Content),
		Tags:        note.Tags,
...
func (h *WritingsHandler) Bookmark(w http.ResponseWriter, r *http.Request) {
	...
	bookmarkContent := BookmarkContent{
		...
		Content:     helpers.RenderMarkdown(bookmark.Description),
		Tags:        bookmark.Tags,
		URL:         bookmark.URL,
