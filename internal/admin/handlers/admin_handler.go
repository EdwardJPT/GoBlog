...
type AdminHandler struct {
	jwt           *jwtauth.JWTAuth
	auth          *admin_models.AdminRepository
...
func NewAdminHandler(db *sql.DB, jwt *jwtauth.JWTAuth) *AdminHandler {
	...
	tmpl = template.Must(tmpl.ParseGlob("internal/admin/templates/admin_login.html"))
	tmpl = template.Must(tmpl.ParseGlob("internal/admin/templates/admin_dashboard_overview.html"))
	...
	return &AdminHandler{
		jwt:           jwt,
		auth:          admin_models.NewAdminRepository(db),
...
func (h *AdminHandler) SignIn(w http.ResponseWriter, r *http.Request) {
	...
	formData := admin_models.SignInData{
	...
	admin_utils.SetTokenCookie(w, tokenString, formData.Remember)
