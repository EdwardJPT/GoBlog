...
func NewSelectedHandler(db *sql.DB) *SelectedHandler {
	...
	tmpl = template.Must(tmpl.ParseGlob("internal/admin/templates/selected-writings/*.html"))
	tmpl = template.Must(tmpl.ParseGlob("internal/admin/templates/selected-works/*.html"))
...
type WritingsListData struct {
	*service.StatsCardData
	Writings    []*admin_models.SelectedWritingsValue
	Filter      string
	SectionType string
}
...
type WorksListData struct {
	*service.StatsCardData
	Works      []*admin_models.SelectedWorksValue
